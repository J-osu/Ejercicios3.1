  ...
  <div class="container my-5">
    ...
    <div class="card p-4 mb-4 shadow-sm">
      ...
      <div v-else-if="coords">
        ...
        <p class="small text-muted mt-2">Radio de Proximidad: {{ proximityRadiusKm }} km</p>
      </div>
    ...
    <div class="card p-4 shadow-lg">
        <h4 class="mb-3 text-secondary">Amigos en el Radar ({{ nearbyFriends.length }})</h4>

        <ul v-if="nearbyFriends.length > 0" class="list-group">
            <li v-for="friend in nearbyFriends" :key="friend.id" class="list-group-item d-flex justify-content-between align-items-center">
...
const nearbyFriends = computed(() => {
    ...
    const userCoords = coords.value;
    
    // Filtramos la lista de amigos
    const nearby = mockFriends.map(friend => {
        // Calcular la distancia entre el usuario y el amigo
        const distance = calculateDistance(userCoords, friend);
        
        // Devolvemos el amigo con la distancia añadida
        return {
            ...friend,
            distance,
            isNear: distance <= proximityRadiusKm,
        };
