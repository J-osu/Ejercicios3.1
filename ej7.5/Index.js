...
var ordenarPorPrecio = __spreadArray([], productos, true).sort(function (a, b) { return a.precio - b.precio; });
...
var ordenarPorFecha = __spreadArray([], productos, true).sort(function (a, b) { return a.fechaDeIngreso - b.fechaDeIngreso; });
...
var filtrarPrecioMenor25 = productos.filter(function (precio) { return precio.precio < 25; });
...
productos.forEach(function (p) {
    return console.log("".concat(p.nombre, ": ").concat(((p.cantidad - 5) / 25 * 100).toFixed(2), "%"));
});
...
var productoscaros = __spreadArray([], productos, true).sort(function (a, b) { return b.precio - a.precio; }).slice(0, 3);
...
var ganancia = productos.filter(function (productos) { return productos.categoria === "Accesorios"; }).reduce(function (acc, p) { return acc + p.precio * p.cantidad; }, 0);
...
var ultimos15Dias = productos.filter(function (p) {
    var fecha = new Date(p.fechaDeIngreso);
...
var zapatos2030 = productos.filter(function (p) { return p.categoria === "Calzado" && p.precio >= 20 && p.precio <= 30; });
console.log(zapatos2030);
// 10. Agrega un nuevo producto
console.log("10. Agrega un nuevo producto");
var nuevoProducto = { nombre: 'Nosealgo', precio: 12, cantidad: 12, categoria: 'Calzado', fechaDeIngreso: '2025-12-04' };
...
var mayorStock = __spreadArray([], productos, true).sort(function (a, b) { return b.cantidad - a.cantidad; }).slice(0, 5);
mayorStock.forEach(function (p) { return p.precio *= 0.95; });
console.log(mayorStock);
// 12. Borra los productos del mes de octubre
console.log("12. Borra los productos del mes de octubre");
var productosSinOctubre = productos.filter(function (p) { return new Date(p.fechaDeIngreso).getMonth() !== 9; });
